...
class VoronoiDiagramGenerator
{
	...
	static int scomp(const void *p1,const void *p2) //utility function
	{
...
int main(int argc,char **argv) 
	{	
		...
		{
			flag=0;
			float xval=rand()%1000;
			float yval=rand()%1000;
			for(int j=0;j<i;j++)
			{
				if(yval==yValues[j])
				{
			...
			if(xval>=50 && xval<=950 && yval>=50 && yval<=950 && flag==0)
			{
				xValues[i]=xval;
				yValues[i]=yval;
				i++;
		...
		{
			cout<<i+1<<" x="<<xValues[i]<<" y="<<yValues[i]<<endl;
		}
		VoronoiDiagramGenerator vdg;
		vdg.generateVoronoi(xValues,yValues,count, 0,1000,0,1000,0.0001);
		...
		fout<<"<rect width=\"1000\" height=\"1000\" style=\"fill:rgb(255,255,255); stroke-width:2; stroke:rgb(255,0,0)\" />"<<endl;
		vector <vector <float>> lines;
	
		while(vdg.getNext(x1,y1,x2,y2))
		{
			printf("Line (%f,%f)->(%f,%f)\n",x1,y1,x2,y2);
			 fout<<"<line x1=\""<<x1<<"\" y1=\""<<y1<<"\" x2=\""<<x2<<"\" y2=\""<<y2<<"\" style=\"stroke:rgb(255,0,0);stroke-width:2\" />"<<endl;
			vector <float> temp;
			temp.push_back(x1);
			temp.push_back(y1);
			temp.push_back(x2);
		...
		float min,a,b,c,num,den,x,y,sqr,xdiff,ydiff;
		...
		for(int i=0;i<count;i++)
		{
			...
			{
				 a=(lines[j][1]-lines[j][3])/(lines[j][2]-lines[j][0]);
				b=1;
				c=-lines[j][2]*a-lines[j][3];
				num= a*xValues[i]+b*yValues[i]+c;
				sqr=a*a+b*b;
				
			
				x=-num*a/(sqr)+xValues[i];
				y=-num*b/(sqr)+yValues[i];
				...
				if(x>=0 && x<=1000 && y>=0 && y<=1000)
				{
					...
					if(x<=xmax+0.0005 && x>=xmin-0.0005 && y<=ymax+0.0005 && y>=ymin-0.0005)
					{			
						//cout<<"i="<<i<<" "<<"j="<<j<<" "<<"x="<<x<<" "<<"y="<<y<<" 						"<<endl;					
						xdiff=x-xValues[i];
						ydiff=y-yValues[i];
						den=xdiff*xdiff+ydiff*ydiff;
						min=sqrt(den);
			...
			if(xValues[i]<radius[i])
			{
				radius[i]=xValues[i];
			}
			if((1000-xValues[i])<radius[i])
			{
				radius[i]=1000-xValues[i];
			}
			if(yValues[i]<radius[i])
			{
				radius[i]=yValues[i];
			}
			if((1000-yValues[i])<radius[i])
			{
				radius[i]=1000-yValues[i];
			}
		
			cout<<"Radius of circle for site "<<i+1<<" = "<<radius[i]<<endl;
		}
		for(int i=0;i<count;i++)
		{
			fout<<"<circle cx=\""<<xValues[i]<<"\" cy=\""<<yValues[i]<<"\" r=\""<<radius[i]<<"\" stroke=\"black\" stroke-width=\"0\" fill=\"#0066ff\" fill-opacity=\"0.4\" />"<<endl;
			fout<<"<circle cx=\""<<xValues[i]<<"\" cy=\""<<yValues[i]<<"\" r=\"2\" stroke=\"black\" stroke-width=\"0\" fill=\"#000000\" />"<<endl;
			fout<<"<text x=\""<<xValues[i]<<"\" y=\""<<yValues[i]<<"\">"<<i+1<<"</text>"<<endl;
		}
